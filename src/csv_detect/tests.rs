...
fn imports_bundled_two_year_demo() {
    ...
    let months =
        crate::analytics::monthly_totals_without_transfers(&outcome.transactions, &[], usize::MAX);
...
fn scoped_test_csv(name: &str, content: &str) -> PathBuf {
    let path = std::env::temp_dir().join(format!("bank-files-{name}-{}.csv", std::process::id()));
