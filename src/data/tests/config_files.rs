...
#[test]
fn editable_budgets_roundtrip_parent_code() {
    let budgets = vec![
        editable_budget("HOME", "", "Housing", "500", "expense", "real"),
        EditableBudget {
            parent_code: "HOME".to_string(),
            ..editable_budget("RENT", "", "Rent", "300", "expense", "real")
        },
    ];
    ...
    assert!(csv.starts_with("code,parent_code,special,category"));
    assert_eq!(parsed[0].parent_code, "");
    assert_eq!(parsed[1].parent_code, "HOME");
}

#[test]
fn editable_budgets_load_old_files_without_parent_code() {
    let parsed = parse_editable_budgets(
        "code,special,category,monthly_budget,yearly_budget,direction,income_basis,notes\nFOOD,,Groceries,10,,expense,real,\n",
    )
    .unwrap();

    assert_eq!(parsed[0].code, "FOOD");
    assert_eq!(parsed[0].parent_code, "");
}

#[test]
fn editable_budgets_reject_invalid_parent_links() {
    let parent = editable_budget("HOME", "", "Housing", "500", "expense", "real");
    let child = EditableBudget {
        parent_code: "MISSING".to_string(),
        ..editable_budget("RENT", "", "Rent", "300", "expense", "real")
    };
    assert!(validate_editable_budgets(&[parent.clone(), child]).is_err());

    let self_parent = EditableBudget {
        parent_code: "HOME".to_string(),
        ..parent.clone()
    };
    assert!(validate_editable_budgets(&[self_parent]).is_err());

    let income_child = EditableBudget {
        parent_code: "HOME".to_string(),
        ..editable_budget("SALARY", "", "Salary", "300", "income", "real")
    };
    assert!(validate_editable_budgets(&[parent.clone(), income_child]).is_err());

    let child = EditableBudget {
        parent_code: "HOME".to_string(),
        ..editable_budget("RENT", "", "Rent", "300", "expense", "real")
    };
    let cycle_parent = EditableBudget {
        parent_code: "RENT".to_string(),
        ..parent
    };
    assert!(validate_editable_budgets(&[cycle_parent, child]).is_err());
}
