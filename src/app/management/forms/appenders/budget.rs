...
pub(in crate::app) fn append_budget_form(
    ...
    let parent_code =
        editable_budget_parent_combo(&budget.parent_code, &budget.code, advanced_features);
    ...
    if is_special_neutral_budget {
        ...
        parent_code.set_sensitive(false);
    ...
    let income_basis_label = if advanced_features {
        let mut row = 0;
        add_labeled(&grid, row, "Code", &code);
        row += 1;
        add_labeled(&grid, row, "Category", &category);
        row += 1;
        let show_parent = !is_special_neutral_budget;
        if show_parent {
            add_labeled(&grid, row, "Parent budget", &parent_code);
            row += 1;
        } else {
            parent_code.set_visible(false);
        }
        add_labeled(&grid, row, "Monthly budget", &monthly_budget);
        row += 1;
        add_labeled(&grid, row, "Yearly budget", &yearly_budget);
        row += 1;
        add_labeled(&grid, row, "Direction", &direction);
        row += 1;
        let income_basis_label = add_labeled(&grid, row, "Percentage basis", &income_basis);
        row += 1;
        add_labeled(&grid, row, "Note", &notes);
    ...
    let update_summary: Rc<dyn Fn()> = {
        ...
        let code = code.clone();
        let parent_code = parent_code.clone();
    ...
    connect_combo_summary(&parent_code, &update_summary);
    ...
    connect_combo_summary(&parent_code, &update_revert_state);
    ...
    let parent_code_for_revert = parent_code.clone();
    ...
    card.revert_button.connect_clicked(move |_| {
        ...
        set_option_combo(&parent_code_for_revert, &original_budget.parent_code);
