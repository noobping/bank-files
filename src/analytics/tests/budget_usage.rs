...
#[test]
fn budget_usage_rolls_child_budgets_into_parent_rows() {
    let rows = vec![
        tx("2025-03-01", -100, "Housing", "HOME"),
        tx("2025-03-02", -700, "Rent", "RENT"),
        tx("2025-03-03", -80, "Utilities", "UTIL"),
    ];
    let budgets = vec![
        fixed_expense_budget("HOME", "Housing"),
        BudgetCode {
    ...
    let usage = budget_usage(&rows, &budgets, MonthKey::new(2025, 3));
    let home = usage.iter().find(|row| row.code == "HOME").unwrap();
    let rent = usage.iter().find(|row| row.code == "RENT").unwrap();

    assert_eq!(home.actual, Decimal::new(880, 0));
    assert_eq!(home.budget, Decimal::new(300, 0));
    assert_eq!(home.remaining, Decimal::new(-580, 0));
    assert_eq!(home.budget_basis, "including sub budgets");
    assert_eq!(rent.actual, Decimal::new(700, 0));
    assert_eq!(rent.budget, Decimal::new(100, 0));
}
