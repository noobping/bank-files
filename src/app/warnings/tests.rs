...
#[test]
fn monthly_warnings_do_not_double_count_parent_budget_rollups() {
    let mut rent = budget("RENT", "Rent", BudgetDirection::Expense, 70);
    rent.parent_code = "HOME".to_string();
    let mut utilities = budget("UTIL", "Utilities", BudgetDirection::Expense, 20);
    utilities.parent_code = "HOME".to_string();
    let data = app_data(
        Vec::new(),
        vec![
            budget("INC", "Income", BudgetDirection::Income, 100),
            budget("HOME", "Home", BudgetDirection::Expense, 10),
            rent,
            utilities,
        ],
    );
...
#[test]
fn annual_warnings_do_not_double_count_parent_budget_rollups() {
    let mut rent = budget("RENT", "Rent", BudgetDirection::Expense, 70);
    rent.parent_code = "HOME".to_string();
    let mut utilities = budget("UTIL", "Utilities", BudgetDirection::Expense, 20);
    utilities.parent_code = "HOME".to_string();
