...
pub(in crate::app) fn show_new_budget_dialog(request: NewBudgetDialogRequest<'_>) {
    ...
    let parent_code = editable_budget_parent_combo("", "", advanced_features);
    ...
    let income_basis_label = if advanced_features {
        ...
        add_labeled(&grid, 2, "Parent budget", &parent_code);
        add_labeled(&grid, 3, "Monthly budget", &monthly_budget);
        add_labeled(&grid, 4, "Yearly budget", &yearly_budget);
        add_labeled(&grid, 5, "Direction", &direction);
        let income_basis_label = add_labeled(&grid, 6, "Percentage basis", &income_basis);
        add_labeled(&grid, 7, "Note", &notes);
        income_basis_label
    } else {
        code.set_visible(false);
        add_labeled(&grid, 0, "Category", &category);
        add_labeled(&grid, 1, "Parent budget", &parent_code);
    ...
    add_button.connect_clicked(move |_| {
        ...
        let budget = refund_budget::normalize_editable_budget(
            transfer_budget::normalize_editable_budget(EditableBudget {
                code: code_text,
                parent_code: combo_active_id(&parent_code),
                special: String::new(),
