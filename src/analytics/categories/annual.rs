...
pub fn annual_budget_usage(
        ...
        .filter(|budget| budget.direction.is_expense())
    {
        ...
        let rollup_codes = budget_rollup_codes(budgets, &budget.code);
        let actual = sum_for_codes(&actual_by_code, &rollup_codes);
        let previous_actual =
            include_previous.then(|| sum_for_codes(&previous_by_code, &rollup_codes));
        let annual_budget = rollup_codes
            .iter()
            .filter_map(|code| budget_by_code(budgets, code))
            .map(|configured| {
                configured.annual_amount_with_basis(real_year_income, planned_year_income)
            })
            .sum();
        let budget_basis = if budget_has_children(budgets, &budget.code) {
            "including sub budgets".to_string()
        } else {
            budget.annual_budget_description()
        };
